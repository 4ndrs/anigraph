...
def print_stuff(conf_path, req, top_n):
    ...
    if req == 'vp':
        ...
        # This takes a while
        print('Calculating the total series (This may take a while)', end='', flush=True)
        _c = 0
        for _va_id in _va_ids:
            _va_ids[_va_id] = con.execute(queries.userdb_get_va_series_count, (_va_id,)).fetchone()[0]
            if _c % 100 == 0: print('.', end='', flush=True) # simple progress bar
            _c += 1
        print('')
